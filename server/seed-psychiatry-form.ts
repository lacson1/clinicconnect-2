...
async function seedPsychiatryForm() {
  try {
    console.log('🌱 Seeding Modern Psychiatry Consultation Form...');

    // Check if form already exists
    const existing = await db
      .select()
      .from(consultationForms)
      .where(eq(consultationForms.name, 'Modern Psychiatry Consultation'))
      .limit(1);

    if (existing.length > 0) {
      console.log('✅ Psychiatry form already exists, updating...');
      await db
        .update(consultationForms)
        .set({
          description: 'Comprehensive psychiatric assessment with structured mental state examination, risk assessment, and evidence-based treatment planning',
          formStructure: psychiatryFormStructure as any,
          isActive: true,
          updatedAt: new Date(),
        })
        .where(eq(consultationForms.name, 'Modern Psychiatry Consultation'));
      console.log('✅ Psychiatry form updated successfully!');
    } else {
      // Get a default user ID (you may want to adjust this)
      const defaultUserId = 1; // Assuming user ID 1 exists

      await db.insert(consultationForms).values({
        name: 'Modern Psychiatry Consultation',
        description: 'Comprehensive psychiatric assessment with structured mental state examination, risk assessment, and evidence-based treatment planning',
        specialistRole: 'Psychiatrist/Doctor',
        createdBy: defaultUserId,
        isActive: true,
        formStructure: psychiatryFormStructure as any,
      });

      console.log('✅ Modern Psychiatry Consultation Form seeded successfully!');
    }
  } catch (error) {
    console.error('❌ Error seeding psychiatry form:', error);
    throw error;
  }
}
